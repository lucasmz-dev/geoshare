...
First, go to Settings > Apps > Maps > _Open by default_ and disable Maps from
...
Then go to Settings > Apps > Geo Share > _Open by default_ > _Add links_ and
enable Geo Share to handle its supported links.
...
Make sure to select at least `maps.google.com` and `maps.app.goo.gl`. If some
...
alt="Screenshot Geo Share's geo: link conversion form"
width="540">
...
It's not possible to share a link from GMaps WV with Geo Share, because GMaps WV
already produces geo: links. So you don't need Geo Share and can directly share
a location from GMaps WV with another map app.
